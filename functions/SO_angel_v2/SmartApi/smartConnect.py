...
import json
import logging
from functions.SO_angel_v2.SmartApi import smartExceptions as ex
import requests
...
import time
import ssl
from functions.SO_angel_v2.SmartApi.version import __version__, __title__
    ...
    def _request(self, route, method, parameters=None):
        ...
        try:
            ...
            if method in ["GET", "DELETE"]:
                r = self.reqsession.request(method, url, params=params, headers=headers, verify=not self.disable_ssl,
                                            timeout=self.timeout, proxies=self.proxies)
            else:
                r = self.reqsession.request(method, url, json=params, headers=headers, verify=not self.disable_ssl,
                                            timeout=self.timeout, proxies=self.proxies)

            if self.debug:
                log.debug("Response: %s", r.content)

            # Decode using 'latin-1' which can handle all byte values
            content = r.content.decode('latin-1')

            try:
                data = json.loads(content)
            except ValueError:
                raise ex.DataException(f"Could not parse the JSON response received from the server: {content}")
            ...
            if data.get("error_type"):
                ...
                # Other errors
                raise ex.SmartAPIException(data["error_type"], data["message"] if "message" in data else None)

            return data
        except Exception as e:
            raise ex.SmartAPIException(str(e))
