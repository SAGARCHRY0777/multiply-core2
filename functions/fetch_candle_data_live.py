from functions.data_utils import (is_within_time_range, get_last_and_start_dates_candle_data_for_symbols,
                                  get_nearest_expiry_commodities, filter_symbol_data)
...
def fetch_candle_data_equities(comm_time, symbols_list, intervals, allowed_intervals,
    ...
    """Fetch equity candle data using Angel One API (faster than TradingView)."""
    print("fetch_candle_data_equities")
    if not symbols_list:
        logging.warning("No equity symbols provided, skipping equity data fetch.")
        return True
    
    # Get equity symbol dicts from all_symbols_data
    from functions.data_utils import filter_symbol_data
    equity_symbol_dicts = filter_symbol_data(all_symbols_data)
    
    max_workers = min(8, len(equity_symbol_dicts))
    ...
    for interval in intervals:
        ...
        # Use Angel One fetcher for ALL equity intervals (faster than TradingView)
        results_cache = {}
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = []
            logging.info(f"Processing {len(equity_symbol_dicts)} equities via Angel One API.")
            num_users = len(login_details_list)
            broker_args = [
                (s, login_details_list[i % num_users], interval, last_processed_date_candle_data_dict,
                 interval, login_details_list)
            ...
            for future in futures:
                try:
                    ...
                    logging.error(f"Error processing equity symbol: {e}")
        ...
        gc.collect()
        logging.info(f"Completed processing {len(results_cache)} equities for {interval}")
        
    return True

def fetch_candle_data_live(config, comm_time, intervals, all_symbols_data, login_details_list,
                           equity_symbols=None):
    print("fetch_candle_data_live")
    is_equity_market_hours = is_within_time_range(time(9, 0), time(15, 30),
                                                  comm_time)
    is_commodity_market_hours = is_within_time_range(time(9, 0), time(23, 59),
                                                     comm_time)
